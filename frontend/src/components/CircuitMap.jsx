    ...
    <div className="trackWrap">
      <svg viewBox="0 0 1000 560" className="trackSvg">
        ...
        {(state.telemetry || []).map((car) => {
          ...
          return (
            <g key={car.console_ip} transform={`translate(${x} ${y})`} opacity={car.connected ? 1 : 0.35}>
      ...
      <p className="hint">La carte utilise les coordonnées monde GT7 projetées. Pour un tracé exact par circuit, il faudra calibrer chaque circuit.</p>
    </div>
  )
}
